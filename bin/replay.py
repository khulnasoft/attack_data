...
def send_to_khulnasoft(settings):
    # connect to khulnasoft
    try:
        service = client.connect(host=settings['khulnasoft']['host'], port=8089, username=settings['khulnasoft']['username'], password=settings['khulnasoft']['password'])
    except ConnectionRefusedError as e:
        print("ERROR - could not connect to the khulnasoft server {}:8089".format(settings['khulnasoft']['host']))
        sys.exit(1)
    ...
    for dataset in settings['datasets']:
        ...
        if dataset['enabled']:
            ...
            if dataset['replay_parameters']['index'] not in service.indexes:
                print("ERROR - index {0} does not exist on khulnasoft server  {1}.".format(dataset['replay_parameters']['index'], settings['khulnasoft']['host']))
                sys.exit(1)
            # set index
...
if __name__ == "__main__":
    # grab arguments
    parser = argparse.ArgumentParser(description="replays attack_data datasets into a configured khulnasoft server", epilog="""
        replay.py requires you to have a running khulnasoft instance and username/password set under replay.yml in order to function.""")
    ...
    status = send_to_khulnasoft(settings)

    if status:
        print("successfully indexed {0} dataset on khulnasoft server {1}".format(len(settings['datasets']), settings['khulnasoft']['host']))
    else:
        print("ERROR - issue replaying desired datasets on khulnasoft server {0}".format(settings['khulnasoft']['host']))
